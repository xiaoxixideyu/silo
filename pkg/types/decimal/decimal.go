...
func NewFromBigRatExact(value *big.Rat) (Decimal, error) {
	if value == nil {
		return Decimal{}, fmt.Errorf("cannot convert nil *big.Rat to decimal")
	}

	// Handle zero case
	if value.Sign() == 0 {
		return Decimal{value: new(big.Int), exp: 0}, nil
	}
	...
	// Handle integer case (denominator is 1)
	if den.Cmp(big.NewInt(1)) == 0 {
		return Decimal{value: new(big.Int).Set(num), exp: 0}, nil
	}

	// For exact conversion, we need to find the exact decimal representation
	// by performing long division and finding the repeating pattern if any.

	// First, let's try to find if the rational number has a finite decimal representation
	// by checking if the denominator only has factors of 2 and 5
	denCopy := new(big.Int).Set(den)

	// Remove all factors of 2
	for denCopy.Bit(0) == 0 {
		denCopy.Rsh(denCopy, 1)
	}

	// Remove all factors of 5
	for {
		quo, rem := new(big.Int).QuoRem(denCopy, big.NewInt(5), new(big.Int))
		if rem.Sign() != 0 {
			break
		}
		denCopy.Set(quo)
	...
	// If denCopy is 1, then the rational number has a finite decimal representation
	if denCopy.Cmp(big.NewInt(1)) == 0 {
		// We can use exact division
		numDecimal := Decimal{value: new(big.Int).Set(num), exp: 0}

		// Use a very high precision to ensure we get the exact result
		precision := int32(1000)
		result := numDecimal.DivRound(Decimal{value: new(big.Int).Set(den), exp: 0}, precision)

		// Verify the result is exact by checking the remainder
		quotient, remainder := numDecimal.QuoRem(Decimal{value: new(big.Int).Set(den), exp: 0}, precision)
		if remainder.IsZero() {
			return quotient, nil
		}

		// If we still have a remainder, this shouldn't happen for finite decimals
		// but we'll return the best approximation
		return result, nil
	}

	// For rational numbers with infinite decimal representation (like 1/3), we need to decide how to handle this. Since we can't represent infinite
	// decimals exactly, we'll use a high precision approximation and return an error
	// indicating that the conversion is not exact.

	numDecimal := Decimal{value: new(big.Int).Set(num), exp: 0}
	denDecimal := Decimal{value: new(big.Int).Set(den), exp: 0}

	// Use high precision for the approximation
	precision := int32(1000)
	result := numDecimal.DivRound(denDecimal, precision)

	return result, fmt.Errorf("rational number %v cannot be exactly represented as a finite decimal", value)
}
...
func (d *Decimal) Scan(value any) error {
	...
	case uint64:
		...
		*d = NewFromUint64(v)
		return nil
	case *big.Rat:
		decimal, err := NewFromBigRatExact(v)
		if err != nil {
			return err
		}
		*d = decimal
